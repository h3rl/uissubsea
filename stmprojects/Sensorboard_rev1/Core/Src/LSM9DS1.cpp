...
void LSM9DS1::readTemp()
{
	...
	traw = offset + ((((int16_t)temp[1] << 8) | temp[0]) >> 8) ;
}
...
{
	HAL_I2C_Mem_Write(hi2c, address, subAddress, 1, &data, 1, 100);
...
uint8_t LSM9DS1::I2CreadByte(uint8_t address, uint8_t subAddress)
{
	...
	uint8_t data = subAddress; // `data` will store the register data

	// send register to device
	HAL_StatusTypeDef ret;
    ret = HAL_I2C_Master_Transmit(&hi2c1, address, &data, 1, 1000);
	if ( ret != HAL_OK ) return ret;

	// recieve data from register
	ret = HAL_I2C_Master_Receive(&hi2c1, address, &data, 1, 1000);
	if ( ret != HAL_OK ) return ret;
...
uint8_t LSM9DS1::I2CreadBytes(uint8_t address, uint8_t subAddress, uint8_t * dest, uint8_t count)
{
	...
	dest[0] = subAddress | 0x80; // `data` will store the register data

	// send register to device
	HAL_StatusTypeDef ret;
    ret = HAL_I2C_Master_Transmit(&hi2c1, address, dest, 1, 1000);
	if ( ret != HAL_OK ) return ret;

	// recieve data from register
	ret = HAL_I2C_Master_Receive(&hi2c1, address, dest, count, 1000);
	if ( ret != HAL_OK ) return ret;
